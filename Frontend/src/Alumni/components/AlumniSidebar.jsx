...
const AlumniSidebar = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`${isSidebarCollapsed ? 'w-16' : 'w-64'} bg-white dark:bg-gray-800 shadow-lg h-screen flex flex-col fixed z-40 transition-all duration-300 ${
          ...
          {menuItems.map((item, index) => (
            item.path ? (
              <Link
                ...
                to={item.path}
                onClick={() => setIsMobileMenuOpen(false)}
                className={`flex items-center justify-between px-4 sm:px-6 py-2 sm:py-3 text-sm font-medium transition-all duration-200 ${
